...
use axum::{
    Json, Router,
    extract::{Request, State, connect_info::IntoMakeServiceWithConnectInfo},
...
use puncture_cli_core::{
    ...
    InviteResponse, ListChannelsResponse, ListPeersResponse, ListUsersResponse, NewAddressResponse,
    NodeIdResponse, OnchainSendRequest, OpenChannelRequest, OpenChannelResponse, PeerInfo,
};
use puncture_core::invite;
...
pub async fn invite(State(state): State<AppState>) -> Result<Json<InviteResponse>, ApiError> {
    Ok(Json(InviteResponse {
        invite: invite::encode(&state.endpoint.node_id()),
    }))
}
...
pub fn router(app_state: AppState) -> IntoMakeServiceWithConnectInfo<Router, SocketAddr> {
